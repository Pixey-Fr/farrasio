...
class RootEnemy {
    constructor(data){
        ...
        switch(data.type){
            ...
            case 'bullet':
                return new Bullet(data.id, data.arena, data.rarity, data.type, data.x, data.y, data.angle);
            case 'homing':
                return new Homing(data.id, data.arena, data.rarity, data.type, data.x, data.y);
            default:
                return new EvadesBall(data.id, {w: 1000, h: 1000}, 'common', 'evadesnormal', data.x, data.y);
        }
    }
}
...
class EvadesBall {
    ...
    simulate(dt, players, enemies, arena){
        ...
        if(this.spawnRarity && this.maxSpawns > 0 && global.totalEnemyArea < 900){
            this.timer -= dt;
            if(this.timer < 0){
...
class DiepEnemy {
    constructor(id, arena, rarity, type, x, y){
        this.id = id;
        this.r = Math.sqrt(scaleFactors[rarity]**1.2)*15-7;
...
class Bullet {
    constructor(id, arena, rarity, type, x, y, angle){
        this.id = id;
        this.x = x||this.r+Math.random()*(arena.w-2*this.r);
        this.y = y||this.r+Math.random()*(arena.h-2*this.r);
